...
const TechCards = ({ children = null, withToggler = false }) => (
  <TechCardsWrapper withToggler={withToggler}>
    {React.Children.map(children, (child, index) => {
      ...
      const { href, title, description, icon, unclickable } = child.props;
      ...
      const isExternal = href && (href.startsWith('http') || !href.includes(DOCS_BASE_PATH));
      // If child.props.unclickable is true or href is missing/empty, render as a div (unclickable)
      const shouldBeUnclickable = unclickable || !href;

      if (shouldBeUnclickable) {
        return (
          <li className="!m-0 before:hidden" key={index}>
            <div
              className={clsx(
