  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Reduce gap between columns by decreasing gap-x-8 to gap-x-4 */}
        <div className="flex w-full max-w-[860px] justify-start gap-x-4 xl:max-w-[700px] lg:hidden">
          {/* Social and Company columns closer, and overall columns shifted left */}
          <div className="flex gap-x-8">
            {MENUS.footer
              .filter(({ heading }) => heading === 'Social' || heading === 'Company')
                            ...
                            {icon && (
                              <span
                                className={clsx(
                                  ...
                                  to &&
                                    'transition-colors duration-200 group-hover/link:bg-green-45'
                                )}
                              />
                            )}
                            {text}
          ...
          {/* Render the rest of the columns as before, excluding Social and Company */}
          {MENUS.footer
            .filter(({ heading }) => heading !== 'Social' && heading !== 'Company')
            .map(({ heading, items }, index) => (
              <div className="flex flex-col pt-3" key={heading}>
                <span
                  className={clsx(
                    'relative text-xs font-semibold uppercase leading-none tracking-normal dark:text-white',
                    isDarkTheme ? 'text-white' : 'text-gray-new-10'
